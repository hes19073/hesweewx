...
"""
  ...
  day_ET and ET24:
    ...
    This extension does the same for ET and provides day_ET and
    ET24.
...
class GTSType(weewx.xtypes.XType):
    ...
    def __init__(self, lat, lon):
        ...
        # ET
        weewx.units.obs_group_dict.setdefault('day_ET', 'group_rain')
    ...
    def get_scalar(self, obs_type, record, db_manager):
        ...
        if obs_type == 'LMTtime':
            ...
            return weewx.units.ValueTuple(datetime.datetime.fromtimestamp(record['dateTime'],
                    self.lmt_tz).strftime("%H:%M:%S"),
                    'unix_epoch', 'group_time')
        
        # This functions handles 'GTS' and 'GTSdate'.
        if obs_type not in ['GTS', 'GTSdate', 'day_ET', 'ET24']:
        # if obs_type not in ['GTS', 'GTSdate']:
            raise weewx.UnknownType(obs_type)
        ...
        if obs_type == 'day_ET':
            # day_ET should be comparable to dayRain, so use the same
            ...
            try:
                ...
                raise weewx.CannotCalculate("day_ET: invalid time")
        ...
        if obs_type == 'GTS':
            if aggregate_type == 'avg':
                ...
                if timespan.stop-timespan.start <= 90000:
                    ...
                    raise weewx.CannotCalculate("%s %s invalid timespan %s %s" % (obs_type,
                                                aggregate_type, timespan.stop-timespan.start,
                                                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timespan.start))))
