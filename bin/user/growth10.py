...
class growth10Generator(weewx.reportengine.ReportGenerator):
    ...
    def __init__(self, config_dict, skin_dict, gen_ts, first_run, stn_info, record=None):
        ...
        self.growth10_report_generator = growth10ReportGenerator(config_dict, skin_dict, gen_ts, first_run, stn_info, record)
        return
    ...
    def run(self):
        ...
        if hasattr(self, 'plot'):
            self.growth10_report_generator.recs = self.zip_vectors()
            self.growth10_report_generator.run()
        return self
        
    def setup(self):
        
        self.image_dict = self.skin_dict['ImageGenerator']
        self.growth10_dict = self.skin_dict['growth10Generator']
    ...
    def plot_image(
        ...
        #offsets = self.growth10_dict[species_name].get('Offsets_from_Biofix')
        #if offsets:
                ...
                for (ndx, (stamp, rec)) in enumerate(recs):
                    if (minstamp <= stamp) and (stamp <= maxstamp):
                        ...
                        try:
                            ...
                            dd = get_growth10(daily_max, daily_min, c_temp_base, c4_temp)
...
# =============================================================================
# growth10ReportGenerator
# =============================================================================

class growth10ReportGenerator(weewx.reportengine.ReportGenerator):
    
    """Class for generating files from growth10 templates.
    ...
    def run(self):
        ...
        """Main entry point for file generation using growth10 Templates.
        ...
        # Look for options in [growth10Generator],
        growth10_dict = gen_dict["growth10Generator"]
        
        # determine how much logging is desired
        log_success = to_bool(growth10_dict.get('log_success', True))

        # configure the search list extensions
        self.initExtensions(growth10_dict)

        # Generate any templates in the given dictionary:
        ngen = ZERO
        for species_name in growth10_dict.sections:
            ngen += self.generate(growth10_dict, species_name)
    ...
    def generate(self, growth10_dict, species_name):
    ...
    def _getSearchList(self, encoding, timespan, default_binding, species_name, report_dict):
        """Get the complete search list to be used by growth10."""
