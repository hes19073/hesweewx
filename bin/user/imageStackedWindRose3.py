...
#                archivedb = self._getArchive(plot_options['archive_database'])
                ...
                for line_name in self.image_dict[timespan][plotname].sections:
                    ...
                    getSqlVectors_TS = weeutil.weeutil.TimeSpan(self.plotgen_ts-int(plot_options.get('time_length', 86400))+1, self.plotgen_ts)
                    (time_vec_t_ws_start, time_vec_t_ws_stop, data_windSpeed) = default_archive.getSqlVectors(getSqlVectors_TS, self.obName)
                    (time_vec_t_wd_start, time_vec_t_wd_stop, data_windDir) = default_archive.getSqlVectors(getSqlVectors_TS, self.dirName)
                    ...
                    textWidth, textHeight = draw.textsize("Wind Rose", font=self.labelFont)
                    if self.label:
                    ...
                    self.roseMaxDiameter = min(int((self.image_height-2*self.windrose_plot_border-labelHeight/2)/22.0)*22,int((self.image_width-(2*self.windrose_plot_border+legendWidth))/22.0)*22)
...
def LegendSetup(self, draw, speedList, speedBin):
    ...
    if self.label:
        ...
        try:
            ...
            draw.text((self.originX-textWidth/2,textHeight/2),self.label.encode("utf-8"), fill=self.windrose_label_font_color, font=self.labelFont)
