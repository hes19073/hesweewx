...
##  xx September 2014   v0.9.4      -fixed error in timespan length for weekdaily
##     (never released)              property in class WDTaggedStats
##                                  -reworked sumQuery def in WDAStatsTypeHelper to
##                                   fix algorithmic error in how 'minute' results
##                                   are calculated
##  7 November 2013     v0.9        -Revised version number to align with Weewx-WD
##                                   version numbering
##                                  -Corrected minor logic errors
##                                  -Strengthened code with try..except statements
##                                   to handle missing historical data
...
class WdArchiveObservationBinder(object):
    ...
    def maxQuery(self):
        ...
        if self.interval <= self.agg_intvl or self.context != 'minute':
            ...
            (start_vt, stop_vt, result_vt) = self.db_lookup().getSqlVectors(getSqlVectors_TS, self.obs_type, 'max', self.agg_intvl)
    ...
    def minQuery(self):
        ...
        if self.interval <= self.agg_intvl or self.context != 'minute':
            getSqlVectors_TS = weeutil.weeutil.TimeSpan(self.timespan[0], self.timespan[1])
            (start_vt, stop_vt, result_vt) = self.db_lookup().getSqlVectors(getSqlVectors_TS, self.obs_type, 'min', self.agg_intvl)
            for elm in result_vt[0]:
    ...
    def avgQuery(self):
        ...
        if self.interval <= self.agg_intvl or self.context != 'minute':
            getSqlVectors_TS = weeutil.weeutil.TimeSpan(self.timespan[0], self.timespan[1])
            (start_vt, stop_vt, result_vt) = self.db_lookup().getSqlVectors(getSqlVectors_TS, self.obs_type, 'avg', self.agg_intvl)
            for elm in result_vt[0]:
                final.append(weewx.units.ValueHelper((elm, result_vt[1], result_vt[2]), self.context,
                                                     self.formatter, self.converter))
        else:
    ...
    def sumQuery(self):
        ...
        if self.interval <= self.agg_intvl or self.context != 'minute':
            # get our results
            getSqlVectors_TS = weeutil.weeutil.TimeSpan(self.timespan[0], self.timespan[1])
            (start_vt, stop_vt, result_vt) = self.db_lookup().getSqlVectors(getSqlVectors_TS, self.obs_type, 'sum', self.agg_intvl)
    ...
    def _do_query(self, aggregateType, val=None):
        ...
        db_manager = self.db_lookup(self.data_binding)
        getSqlVectors_TS = weeutil.weeutil.TimeSpan(self.timespan[0], self.timespan[1])
        (start_vt, stop_vt, result_vt) = db_manager.getSqlVectors(getSqlVectors_TS, self.obs_type, aggregateType, self.agg_intvl)
        # Wrap the result in a ValueHelper:
