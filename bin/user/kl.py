                ...
                if last_ts is not None:
                    ...
                    for y in range(0, 9):
                        if 'Temp%d' % y in r and 'Humidity%d' % y in r:
                            r['Dewpoint%d' % y] = weewx.wxformulas.dewpointC(r['Temp%s' % y], r['Humidity%d' % y])
                            r['Heatindex%d' % y] = weewx.wxformulas.heatindexC(r['Temp%s' % y], r['Humidity%d' % y])
                            r['Heatdeg%d' % y] = weewx.wxformulas.heating_degrees(r['Temp%s' % y], 18.333)
                            r['Cooldeg%d' % y] = weewx.wxformulas.cooling_degrees(r['Temp%s' % y], 18.333)
                            r['Homedeg%d' % y] = weewx.wxformulas.heating_degrees(r['Temp%s' % y], 15.0)
                            r['Absolut%d' % y] = weewx.wxformulas.absF_C(r['Temp%s' % y], r['Humidity%d' % y])
                            r['Avd%d' % y] = (weewx.wxformulas.SaturationVaporPressure(r['Temp%s' % y])) * r['Humidity%d' % y] / 100.0
                            r['Svd%d' % y] = weewx.wxformulas.SaturationVaporPressure(r['Temp%s' % y])
