            ...
            if _row[0] > 0 or _row[1] >0:
                ...
                if ans == 'y':
                    ...
                    for _rec in dbmanager_wx.genBatchRecords(start_ts - 1, stop_ts):
                        ...
                        dbmanager_wx.updateValue(_rec['dateTime'], 'dewpoint', weewx.wxformulas.dewpointC(_rec['outTemp'], _rec['outHumidity']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'inDewpoint',  weewx.wxformulas.dewpointC(_rec['inTemp'], _rec['inHumidity']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'windchill',  weewx.wxformulas.windchillC(_rec['outTemp'], _rec['windSpeed']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'heatindex',  weewx.wxformulas.heatindexC(_rec['outTemp'], _rec['outHumidity']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'humidex', weewx.wxformulas.humidexC(_rec['outTemp'], _rec['outHumidity']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'appTemp', weewx.wxformulas.apptempC(_rec['outTemp'], _rec['outHumidity'], _rec['windSpeed']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'airDensity', weewx.wxformulas.density_Metric(_rec['dewpoint'], _rec['outTemp'], _rec['pressure']))
                        dbmanager_wx.updateValue(_rec['dateTime'], 'windDruck', weewx.wxformulas.winddruck_Metric(_rec['dewpoint'], _rec['outTemp'], _rec['pressure'], _rec['windSpeed']))
