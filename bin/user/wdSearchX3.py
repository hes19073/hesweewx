...
##                                  -added heatColorWord, feelsLike and density
##                                   tags to wdSundryTags SLE
##                                  -added day_windrun, yest_windrun,
##                                   week_windrun, seven_day_windrun,
##                                   month_windrun, year_windrun tags and
##                                   alltime_windrun tags to wdWindRunTags SLE
##  10 January 2015     v1.0.0      -rewritten for Weewx v3.0.0
##                                  -added wdManualAverages SLE
##                                  -fixed issues with wdRainThisDay SLE affecting
##                                   databases with limited historical data
##                                  -fixed bug in wdTimeSpanTags that was
##                                   causing unit issues with $alltime tags
##                                  -removed use of total_seconds() attribute
##                                   in wdRainThisDay
...
##  dd September 2014   v0.9.4      -added execution time debug messages for all SLEs
##    (never released)              -added additional tags to wdMonthStats SLE
##                                  -wdClientrawAgotags and wdTesttagsAgotags SLEs
##                                   now use max_delta on archive queries
##                                  -added additional tags to wdMaxAvgWindTags SLE
##                                  -wdSundryTags SLE now provides current_text and
##                                   current_icon from current conditions text file
##                                   if it exists
##                                  -added additional tags to wdWindRunTags SLE
##                                  -new SLEs wdGdDays, wdForToday, wdRainThisDay
##                                   and wdRainDays
##                                  -added helper functions get_first_day and doygen
##                                  -added GNU license text
##  August 2013         v0.1        -initial implementation
##
import datetime
import time
import os.path
...
class wdMonthStats(SearchList):
    ...
    def getMonthAveragesHighs(self, timespan, db_lookup):
        ...
        monthRainNowBin = [[0 for x in range(2)] for x in range(12)]
        monthTempBin = [[0 for x in range(2)] for x in range(12)]
        monthTempNowBin = [[0 for x in range(2)] for x in range(12)]
        ...
        if (_start_dt.hour != 0 or _start_dt.minute != 0 or _start_dt.day != 1) and ((_start_dt.month == _end_dt.month and _start_dt.year == _end_dt.year) or (_end_dt < datetime.datetime.combine(get_first_day(_start_dt,0,2), _mn_time))):
            ...
            for monthNum in range (12):
                ...
                monthRainAvgNow_vh[monthNum] = ValueHelper(monthRainAvgNow[monthNum], formatter=self.generator.formatter, converter=self.generator.converter)
                ...
                monthTempAvgNow_vh[monthNum] = ValueHelper(monthTempAvgNow[monthNum], formatter=self.generator.formatter, converter=self.generator.converter)
                monthTempMax_vh[monthNum] = ValueHelper(monthTempMax[monthNum], formatter=self.generator.formatter, converter=self.generator.converter)
                monthTempMin_vh[monthNum] = ValueHelper(monthTempMin[monthNum], formatter=self.generator.formatter, converter=self.generator.converter)
            ...
            monthTempMax_vh[_start_dt.month - 1] = ValueHelper(monthTempMax_tuple, formatter=self.generator.formatter, converter=self.generator.converter)
            monthTempMin_vh[_start_dt.month - 1] = ValueHelper(monthTempMin_tuple, formatter=self.generator.formatter, converter=self.generator.converter)
            # Do we have a 2nd month to process
            if (_end_dt < datetime.datetime.combine(get_first_day(_start_dt,0,2), _mn_time)): # we do cross a month boundary
                # Process max/min for month containing _end_ts
                ...
                monthTempMin_tuple = db_lookup().getAggregate(month_timespan, 'outTemp', 'min')
                # Save our max/min to the correct month bin
                monthTempMax_vh[_end_dt.month - 1] = ValueHelper(monthTempMax_tuple, formatter=self.generator.formatter, converter=self.generator.converter)
                monthTempMin_vh[_end_dt.month - 1] = ValueHelper(monthTempMin_tuple, formatter=self.generator.formatter, converter=self.generator.converter)
            monthRainMax_vh[monthNum] = ValueHelper((monthRainMax[monthNum], rain_type, rain_group), formatter=self.generator.formatter, converter=self.generator.converter)
            ymaxrainmonth = None
            ymaxrainyear = None
            yearmaxmonthrain_vh = ValueHelper((None, rain_type, rain_group), formatter=self.generator.formatter, converter=self.generator.converter)
        else:
            ...
            for monthNum in range (12):
                ...
                monthRainMax_vh[monthNum] = ValueHelper((monthRainMax[monthNum], rain_type, rain_group), formatter=self.generator.formatter, converter=self.generator.converter)
                monthTempMax_vh[monthNum] = ValueHelper((monthTempMax[monthNum], outTemp_type, outTemp_group), formatter=self.generator.formatter, converter=self.generator.converter)
                monthTempMin_vh[monthNum] = ValueHelper((monthTempMin[monthNum], outTemp_type, outTemp_group), formatter=self.generator.formatter, converter=self.generator.converter)
                # Save our 'now' results as ValueHelpers
                monthRainAvgNow_vh[monthNum] = ValueHelper((monthRainAvgNow[monthNum], rain_type, rain_group), formatter=self.generator.formatter, converter=self.generator.converter)
                monthTempAvgNow_vh[monthNum] = ValueHelper((monthTempAvgNow[monthNum], outTemp_type, outTemp_group), formatter=self.generator.formatter, converter=self.generator.converter)
            ...
            yearmaxmonthrain_vh = ValueHelper((ymaxrain, rain_type, rain_group), formatter=self.generator.formatter, converter=self.generator.converter)
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        search_list_extension = {'avrainjan' : monthRainAvg_vh[0], 'avrainfeb' : monthRainAvg_vh[1],
                                 ...
                                 'currentmonthavrain' : monthRainAvg_vh[curr_month - 1], 'currentmonthrecordrain' : monthRainMax_vh[curr_month - 1],
                                 'yearmaxmonthrain' : yearmaxmonthrain_vh, 'yearmaxmonthrainmonth' : ymaxrainmonth, 'yearmaxmonthrainyear' : ymaxrainyear,
...
class wdMaxAvgWindTags(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        (windDir_type, windDir_group) = getStandardUnitType(current_rec['usUnits'], 'windDir')
        ...
        # Wrap results in a ValueHelper to provide formatting and unit info
        yest_max_avg_wind_vt = (yest_max_avg_wind, windSpeed_type, windSpeed_group)
        ...
        yest_max_avg_wind_dir_vt = (yest_max_avg_dir, windDir_type, windDir_group)
        ...
        yest_max_avg_wind_time_vt = (yest_max_avg_wind_time, dateTime_type, dateTime_group)
        ...
        avwind120_vh = ValueHelper((avwind120_vt[0][0], avwind120_vt[1], avwind120_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        avwind60_vh = ValueHelper((avwind60_vt[0][0], avwind60_vt[1], avwind60_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        avwind30_vh = ValueHelper((avwind30_vt[0][0], avwind30_vt[1], avwind30_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        avwind15_vh = ValueHelper((avwind15_vt[0][0], avwind15_vt[1], avwind15_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        avwind10_vh = ValueHelper((avwind10_vt[0][0], avwind10_vt[1], avwind10_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        avdir10_vh = ValueHelper((avdir10, windDir_type, windDir_group), formatter=self.generator.formatter, converter=self.generator.converter)
...
class wdMaxWindGustTags(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        (time_start_vt, time_stop_vt, wind_gust_vt) = db_lookup().getSqlVectors(TimeSpan(timespan.stop-86400, timespan.stop), 
                                                                                'windGust')
        ...
        (time_start_vt, time_stop_vt, wind_gust_vt) = db_lookup().getSqlVectors(TimeSpan(timespan.stop-600, timespan.stop), 
                                                                                'windGust', 'max', 600)
...
class wdSundryTags(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        # Get current records from the archive
        if not self.generator.gen_ts:
            self.generator.gen_ts = db_lookup().lastGoodStamp()
        ...
        if outHumidity is not None and outTemp_C is not None:
          cbi = round((((110 - 1.373 * outHumidity) - 0.54 * (10.20 - outTemp_C)) * (124 * 10**(-0.0142 * outHumidity)))/60,1)
        else:
          cbi = 0
        ...
        t2 = time.time()
        logdbg2("wdSundryTags SLE wurde Hergestllt unter Verwendung executed in %0.3f seconds" % (t2-t1))
...
class wdWindRunTags(SearchList):
    """ Search list extension to return windrun over variou speriods. Also
        ...
        loop/archive record. This cumulative alue is reset at midnight each
...
class wdHourRainTags(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        # do not exist
        try:
            (time_start_vt, time_stop_vt, rain_vt) = db_lookup().getSqlVectors(TimeSpan(start_ts, end_ts), 
                                                                               'rain')
        except:
        ...
        max_hour_rain_vh = ValueHelper((max_hour_rain, rain_vt[1], rain_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        max_hour_rain_time_vh = ValueHelper((max_hour_rain_ts, time_stop_vt[1], time_stop_vt[2]), formatter=self.generator.formatter, converter=self.generator.converter)
        # Create a small dictionary with the tag names (keys) we want to use
...
class wdForToday(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        for _ts in doygen(_start_ts, _stop_ts):
            ...
            _row = db_lookup().getSql("SELECT datetime, max, min FROM archive_day_outTemp WHERE dateTime >= ? AND dateTime < ?", (_ts, _ts + 86399))
            if _row is not None:
                # update our max temp and timestamp if necessary
                if _max_temp_today == None:
                    if _row[1] is not None:
        ...
        _max_temp_today_vh = ValueHelper(_max_temp_today_vt, formatter=self.generator.formatter, converter=self.generator.converter)
        _min_temp_today_vh = ValueHelper(_min_temp_today_vt, formatter=self.generator.formatter, converter=self.generator.converter)
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        if _first_good_ts <= _stop_archive_ts:
            ...
            if today_seconds != 0:  # ie it's not midnight
                ...
                (time_start_vt, time_stop_vt, rain_vt) = db_lookup().getSqlVectors(TimeSpan(_start_archive_ts, _stop_archive_ts), 
                                                                                   'rain', 'sum', archive_agg)
            else:
        ...
        if rain_vt[0] != []:
            ...
            year_rain_vh = ValueHelper(year_rain_vt, formatter=self.generator.formatter, converter=self.generator.converter)
        else:
            year_rain_vh = ValueHelper((None, None, None), formatter=self.generator.formatter, converter=self.generator.converter)
