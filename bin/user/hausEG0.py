...
class HausEG(StdService):
    ...
    def read_file(self, event):
        ...
        try:
            ...
            elePV_val = float(values[1])
            event.record['elePVTotal'] = calculate_delta(elePV_val, last_elp)
            elp_new = event.record['elePV']

            f.close()

            dat_A = open("/home/weewx/archive/0ZahlStand", "w")
            data_new = str(was_val) + "," + str(wasA_val) + "," + str(ele_val) + "," + str(gas_val) + "," + str(eleA_val) + "," + str(elePV_val)
