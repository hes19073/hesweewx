# forecast.py 1281 2016-02-07 12:16:04Z hes mwall $
...
[Forecast]
    ...
    [[XWuh]]
        # Location for which forecast TXT are desired
        #location = Boston

        # How often to generate the TXT forecast, in seconds
        #interval = 1209600
...
[Engine]
    [[Services]]
        ...
        archive_services = ... , user.forecast.ZambrettiForecast, user.forecast.WUForecast
...
Forecast Text
  ...
  The index is the event from the current time.

$forecast.xwuh(0).dateTime     date/time that the forecast was requested
$forecast.xwuh(0).issued_ts    date/time that the forecast was created
$forecast.xwuh(0).event_ts     date/time of the event
$forecast.xwuh(0).fcttext_metric TXT forecast
$forecast.xwuh(0).title        day or night 
$forecast.xwuh(0).location     where the is forecast

for wuh in $forecast.xwuhs
  $wuh.event_ts $wuh.hilo $wuh.offset

Forecast Local

  The index is the event from the current time.

$forecast.xwul(0).dateTime     date/time that the forecast was requested
$forecast.xwul(0).issued_ts    date/time that the forecast was created
$forecast.xwul(0).event_ts     date/time of the event
$forecast.xwul(0).fcttext      weather today text
$forecast.xwul(0).title        visibility in km 
...
def get_default_binding_dict():
    return {'database': 'forecast_sqlite',
...
wuh_label_dict = {
 ...
 'tstorms'        : 'tsra',
 'unknown'        : 'na',
 'nt_chanceflurries' : 'nsn',
 'nt_chancerain'    : 'hi_nshwrs',
 'nt_chancesleet'   : 'ip',
 'nt_chancesnow'    : 'nsn',
 'nt_chancetstorms' : 'hi_ntsra',
 'nt_clear'         : 'nskc',
 'nt_cloudy'        : 'novc',
 'nt_flurries'      : 'nsn',
 'nt_fog'           : 'nfg',
 'nt_hazy'          : 'nfg',
 'nt_mostlycloudy'  : 'nbkn',
 'nt_mostlysunny'   : 'nsct',
 'nt_partlycloudy'  : 'nsct',
 'nt_partlysunny'   : 'nbkn',
 'nt_rain'          : 'nra',
 'nt_sleet'         : 'ip',
 'nt_sleat'         : 'ip',
...
def WUDownloadForecast(api_key, location,
    ...
    u = '%s/%s/%s/lang:DL/q/%s.json' % (url, api_key, fc_type, location) \
...
def WUCreateRecordsFromHourly(fc, issued_ts, now, location=None):
    ...
    for period in fc['hourly_forecast']:
        try:
            ...
            r['feelslike'] = str2float('feelsliket', period['feelslike']['english'])
            r['mslp'] = str2float('mslp', period['mslp']['english'])
    ...
    cnt1 = 0
    for period2 in fc['forecast']['txt_forecast']['forecastday']:
        """create from forecast10day"""
        try:
            cnt1 += 1
            rr = {}
            rr['method'] = WU_KEY1
            rr['usUnits'] = weewx.US
            rr['dateTime'] = now
            rr['issued_ts'] = issued_ts
            rr['duration'] = 24 * 3600
            rr['txtperiod'] = str2float('period', period2['period'])
            rr['clouds'] = WU_SKY_DICT.get(period2['icon'], None)
            rr['icon'] = period2['icon']
            rr['title'] =  period2['title']
            rr['fcttext_metric'] =  period2['fcttext_metric']
            rr['pop'] = str2int('pop', period2['pop'])

            records.append(rr)

        except Exception, e:
            msg = '%s: failure in Text forecast period %d: %s' % (
                WU_KEY1, cnt1, e)
            msgs.append(msg)
            logerr(msg)

    # Current infos Sicht in km und Wetter allg
    cnta = 9
    try:
        period3 = fc['current_observation']
        rc = {}
        rc['method'] = WU_KEY2
        rc['usUnits'] = weewx.US
        rc['dateTime'] = now
        rc['issued_ts'] = issued_ts
        rc['event_ts'] = str2int('local_epoch', period3['local_epoch'])
        rc['duration'] = 24 * 3600
        rc['offset'] = str2float('visibility_km', period3['visibility_km'])
        rc['fcttext'] =  period3['weather']

        records.append(rc)

    except Exception, e:
        msg = '%s: failure in current Conditions period %d: %s' % (
                WU_KEY2, cnta, e)
        msgs.append(msg)
        logerr(msg)
...
def WUCreateRecordsFromDaily(fc, issued_ts, now, location=None):
    ...
    records = []
    cnt = 0
    for period in fc['forecast']['txt_forecast']['forecastday']:
        try:
            cnt += 1
            r = {}
            r['method'] = WU_KEY
            ...
            r['duration'] = 24 * 3600
            r['clouds'] = WU_SKY_DICT.get(period['icon'], None)
...
UNIT_GROUPS = {
    ...
    'mslp':         'group_pressure',
    'mslpMin':      'group_pressure',
    'mslpMax':      'group_pressure',
    }
...
class ForecastVariables(SearchList):
    ...
    def __init__(self, generator):
        ...
        self.labels['Wuh'] = dict(wuh_label_dict.items() + label_dict.get('Wuh', {}).items())
        self.labels['Wul'] = dict(wul_label_dict.items() + label_dict.get('Wul', {}).items())
    ...
    def _getWuls(self, context, from_ts=None, max_events=1):
        dbm_dict = weewx.manager.get_manager_dict(
        ...
        with weewx.manager.open_manager(dbm_dict) as dbm:
            ...
            sql = "select dateTime,issued_ts,event_ts,offset,usUnits,fcttext from %s where method = 'WUL' and dateTime = (select max(dateTime) from %s where method = 'WUL')" % (dbm.table_name, dbm.table_name)
            #sql = "select dateTime,issued_ts,event_ts,offset,usUnits,fcttext from %s where method = 'WUL' order by dateTime desc limit 1" % dbm.table_name
            ...
            for count in range(self.db_max_tries):
                try:
                    ...
                    logerr('get Locale failed (attempt %d of %d): %s' %
    ...
    def _getWuhs(self, context, from_ts=None, max_events=1):
        dbm_dict = weewx.manager.get_manager_dict(
            self.generator.config_dict['DataBindings'],
            self.generator.config_dict['Databases'],
            self.binding,
        ...
        with weewx.manager.open_manager(dbm_dict) as dbm:
            if from_ts is None:
                from_ts = int(time.time())
            sql = "select dateTime,issued_ts,event_ts,txtperiod,icon,title,usUnits,fcttext_metric,pop from %s where method = 'WUH' and dateTime = (select max(dateTime) from %s where method = 'WUH') order by txtperiod asc" % (dbm.table_name, dbm.table_name)
                  #, from_ts)
            if max_events is not None:
                sql += ' limit %d' % max_events
            for count in range(self.db_max_tries):
                try:
                    ...
                    for rec in dbm.genSql(sql):
                        ...
                        r['txtperiod'] = rec[3]
                        code = rec[4]
                        text = self.labels['Wuh'].get(code, code)
                        r['icon'] = text
                        r['title'] = rec[5]
                        r['fcttext_metric'] = rec[7]
                        r['pop'] = str(int(rec[8]))
                        records.append(r)
                    return records
                except Exception, e:
                    logerr('get forecast_text failed (attempt %d of %d): %s' %
                           ((count + 1), self.db_max_tries, e))
                    logdbg('waiting %d seconds before retry' %
                           self.db_retry_wait)
                    time.sleep(self.db_retry_wait)
    ...
    def xwul(self, index, from_ts=None):
        if from_ts is None:
            from_ts = int(time.time())
        records = self._getWuls('xwul', from_ts=from_ts, max_events=1)
    ...
    def xwuh(self, index, from_ts=None):
        if from_ts is None:
            from_ts = int(time.time())
        records = self._getWuhs('xwuh', from_ts=from_ts, max_events=index + 1)
        if 0 <= index < len(records):
            return records[index]
        return {'dateTime': '', 'issued_ts': '', 'event_ts': '',
                'icon': '', 'title': '',  'fcttext_metric': '', 'txtperiod': ''}

    def xwuhs(self, from_ts=None, max_events=14, startofday=False):
        """The text forecast returns text events into the future from the
        indicated time using the latest text forecast.

        from_ts - timestamp in epoch seconds.  if nothing is specified, the
                  current time is used.

        max_events - maximum number of events to return.  default to 14 days'
                     worth of text."""
        if from_ts is None:
            from_ts = int(time.time())
        if startofday:
            from_ts = weeutil.weeutil.startOfDay(from_ts)
        records = self._getWuhs('xwuhs', from_ts=from_ts, max_events=max_events)
        return records
    ...
    def weather_summary(self, fid, ts=None, periods=None):
        ...
        rec = {
            ...
            'mslp': None, 'mslpMin': None, 'mslpMax': None,
        ...
        if periods is not None:
            for p in periods:
                if from_ts <= p['event_ts'].raw <= from_ts + dur:
                    ...
                    for s in ['temp', 'uvIndex', 'feelslike', 'mslp', 'dewpoint', 'humidity', 'windSpeed']:
                        _get_stats(s, p, rec)
