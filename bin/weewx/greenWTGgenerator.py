...
def get_green_gdd(d_temp_max, d_temp_min):
    # d_temp_max in degree_F also 10 degree_C = 50.0 degree_F
    # d_temp_min in degree_F also 30 degree_C = 88.0 degree_F
    day_green = 0.0
    d_temp_ma = FtoC(d_temp_max)
    d_temp_na = FtoC(d_temp_min)
    ...
    if d_temp_na < 10.0:
        ...
        if d_temp_ma < 30.0:
            _gdd = (d_temp_ma + d_temp_na) / 2.0 - 10.0
...
def logmsg(lvl, msg):
    syslog.syslog(lvl, 'GreenWenerator: %s' % msg)
...
# =============================================================================
#                    Class GreenWenerator
# =============================================================================

class GreenWenerator(weewx.reportengine.ReportGenerator):
    ...
    def setup(self):
        ...
        self.Green_dict = self.skin_dict['GreenWenerator']
...
#            html_file = os.path.join(image_root, '%s.html' % species_name)
            ...
            if skipThisPlot(plotgen_ts, ai, img_file):
                ...
                try:
                    ...
                    syslog.syslog(syslog.LOG_CRIT, "GreenWenerator: Unable to save to file '%s' %s:" % (img_file, e))
                t2 = time.time()
                if self.log_success:
                    syslog.syslog(syslog.LOG_INFO, "GreenWenerator: Generated %d images for %s in %.2f seconds" % \
    ...
    def plot_image(
        ...
        if biofix:
            ...
            biofix_label = 'Wachstumsgradtage (GDD Basis 10)'
    ...
    def get_vectors(self, stamps, csv_file_name):
        ...
        try:
                ...
                for (ndx, (stamp, rec)) in enumerate(recs):
                    if (minstamp <= stamp) and (stamp <= maxstamp):
                        ...
                        try:
                            ...
                            d_temp_max = float(rec.get('TMPMAX_F'))
                            d_temp_min = float(rec.get('TMPMIN_F'))
                            dd = get_green_gdd(d_temp_max, d_temp_min)
            ...
            syslog.syslog(syslog.LOG_CRIT, "greenWTGgenerator: Unable to read file '%s' %s:" % (csv_file_name, e))
        return result
...
class GreenReportGenerator(weewx.reportengine.ReportGenerator):
    ...
    def run(self):
        ...
        # Look for options in [GreenWenerator],
        Green_dict = gen_dict["GreenWenerator"]
...
#        default_archive = self.db_binder.get_manager(default_binding)
    ...
    def _getSearchList(self, encoding, timespan, default_binding, species_name, report_dict):
        """Get the complete search list to be used by Green."""
