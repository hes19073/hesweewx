...
"""Generate a *.png graph image and an *.html report for the
   Growing Degree Day for the year.
...
def get_growth(d_temp_maxF, d_temp_minF):
    # d_temp_max in degree_F ---- 10 degree_C = 50.0 degree_F
    # d_temp_min in degree_F ---- 30 degree_C = 88.0 degree_F

    d_temp_base = 10.0   # 50 F
    c4_plants = 30.0     # 86 F
    ...
    if d_temp_min < d_temp_base:
        if d_temp_max < c4_plants:
            if d_temp_max < d_temp_base:
                ...
                growth = (d_temp_max + d_temp_base) / 2.0 - d_temp_base
        ...
        if d_temp_max < c4_plants:
            growth = (d_temp_max + d_temp_min) / 2.0 - d_temp_base

        else:
            growth = (c4_plants + d_temp_min) / 2.0 - d_temp_base
...
def logmsg(lvl, msg):
    syslog.syslog(lvl, 'GrowthGenerator: %s' % msg)
...
class GrowthGenerator(weewx.reportengine.ReportGenerator):
    ...
    def genImages(self, gen_ts):
        ...
        for species_name in self.growth_dict.sections:
            ...
            species_options = weeutil.weeutil.accumulateLeaves(self.growth_dict[species_name])
            ...
            if skipThisPlot(plotgen_ts, ai, img_file):
                ...
                t2 = time.time()
                if self.log_success:
                    syslog.syslog(syslog.LOG_INFO, "GrowthGenerator: Generated %d images for %s in %.2f seconds" % \
    ...
    def plot_image(
        ...
        # biofix_actual = None (look at skin)
        horizons = []
        biofix = get_float_t(line_options.get('biofix_actual'), 'group_temperature')
        if biofix:
            biofix_label = 'Wachstum (GrowthDay)'
        else:
            biofix = get_float_t(line_options.get('biofix_estimated', [50, 'degree_F']), 'group_temperature')
            biofix_label = 'Wachstum Beginn (GDD)'
                ...
                for (ndx, (stamp, rec)) in enumerate(recs):
                    if (minstamp <= stamp) and (stamp <= maxstamp):
                        ...
                        try:
                            ...
                            dd = get_growth(daily_max, daily_min)
...
class GrowthReportGenerator(weewx.reportengine.ReportGenerator):
    
    """Class for generating files from Growth templates.
    ...
    def run(self):
        ...
        """Main entry point for file generation using Growth Templates.
        ...
        # Look for options in [GrowthGenerator],
        growth_dict = gen_dict["GrowthGenerator"]
        
        # determine how much logging is desired
        log_success = to_bool(growth_dict.get('log_success', True))

        # configure the search list extensions
        self.initExtensions(growth_dict)

        # Generate any templates in the given dictionary:
        ngen = ZERO
        for species_name in growth_dict.sections:
            ngen += self.generate(growth_dict, species_name)
...
#            self.outputted_dict[key] = []; 
    ...
    def generate(self, growth_dict, species_name):
        ...
        report_dict = weeutil.weeutil.accumulateLeaves(growth_dict[species_name])
