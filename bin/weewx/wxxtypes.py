...
#############################################################################
    ...
    def calc_airDensity(key, data, db_manager=None):
        ...
        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.density_US(data['dewpoint'], data['outTemp'], data['pressure'])
            u = 'kg_per_meter_qubic'
        else:
            val = weewx.wxformulas.density_Metric(data['dewpoint'], data['outTemp'], data['pressure'])
            u = 'kg_per_meter_qubic'
        return weewx.units.convertStd((val, u, 'group_druck3'), data['usUnits'])

    @staticmethod
    def calc_windDruck(key, data, db_manager=None):
        if 'dewpoint' not in data or 'outTemp' not in data or 'windSpeed' not in data or 'pressure' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.winddruck_US(data['dewpoint'], data['outTemp'], data['pressure'], data['windSpeed'])
            u = 'N_per_meter_squared'
        else:
            val = weewx.wxformulas.winddruck_Metric(data['dewpoint'], data['outTemp'], data['pressure'], data['windSpeed'])
            u = 'N_per_meter_squared'
        return weewx.units.convertStd((val, u, 'group_druck2'), data['usUnits'])
    ...
    def calc_wetBulb(key, data, db_manager=None):
        ...
        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.wetbulb_US(data['outTemp'], data['outHumidity'], data['pressure'])
            u = 'degree_F'
        else:
            val = weewx.wxformulas.wetbulb_Metric(data['outTemp'], data['outHumidity'], data['pressure'])
            u = 'degree_C'
        return weewx.units.convertStd((val, u, 'group_temperature'), data['usUnits'])

    @staticmethod
    def calc_deltaT(key, data, db_manager=None):
        if 'outTemp' not in data or 'outHumidity' not in data or 'pressure' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.deltaT_US(data['outTemp'], data['outHumidity'], data['pressure'])
            u = 'degree_F'
        else:
            val = weewx.wxformulas.deltaT_Metric(data['outTemp'], data['outHumidity'], data['pressure'])
            u = 'degree_C'
        return weewx.units.convertStd((val, u, 'group_temperature'), data['usUnits'])

    @staticmethod
    def calc_cbIndex(key, data, db_manager=None):
        if 'outTemp' not in data or 'outHumidity' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.cbindex_US(data['outTemp'], data['outHumidity'])
            u = 'count'
        else:
            val = weewx.wxformulas.cbindex_US(data['outTemp'], data['outHumidity'])
            u = 'count'
        return weewx.units.convertStd((val, u, 'group_count'), data['usUnits'])

    @staticmethod
    def calc_sunshineS(key, data, db_manager=None):
        if 'radiation' not in data or 'dateTime' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.sunhes(data['radiation'], data['dateTime'])
            u = 'second'
        else:
            val = weewx.wxformulas.sunhes(data['radiation'], data['dateTime'])
            u = 'second'
        return weewx.units.convertStd((val, u, 'group_elapsed'), data['usUnits'])

    @staticmethod
    def calc_absolutF(key, data, db_manager=None):
        if 'outTemp' not in data or 'outHumidity' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.absF_F(data['outTemp'], data['outHumidity'])
            u = 'g_per_meter_qubic'
        else:
            val = weewx.wxformulas.absF_C(data['outTemp'], data['outHumidity'])
            u = 'g_per_meter_qubic'
        return weewx.units.convertStd((val, u, 'group_gram'), data['usUnits'])

    @staticmethod
    def calc_dampfDruck(key, data, db_manager=None):
        if 'outTemp' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.dampfD_F(data['outTemp'])
            u = 'inHg'
        else:
            val = weewx.wxformulas.dampfD_F(data['outTemp'])
            u = 'mbar'
    ...
    @staticmethod
    def calc_summersimmerIndex(key, data, db_manager=None):
        if 'outTemp' not in data or 'outHumidity' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.sumsimIndex_F(data['outTemp'], data['outHumidity'])
            u = 'degree_F'
        else:
            val = weewx.wxformulas.sumsimIndex_C(data['outTemp'], data['outHumidity'])
            u = 'degree_C'
        return weewx.units.convertStd((val, u, 'group_temperature'), data['usUnits'])

    @staticmethod
    def calc_densityA(key, data, db_manager=None):
        # if 'outTemp' not in data or 'altimeter' not in data:
        if 'outTemp' not in data or 'pressure' not in data:
            raise weewx.CannotCalculate(key)

        if data['usUnits'] == weewx.US:
            # val = weewx.wxformulas.da_US(data['outTemp'], data['altimeter'])
            val = weewx.wxformulas.da_US(data['outTemp'], data['pressure'])
            u = 'foot'
        else:
            # val = weewx.wxformulas.da_Metric(data['outTemp'], data['altimeter'])
            val = weewx.wxformulas.da_Metric(data['outTemp'], data['pressure'])
            u = 'meter'
        return weewx.units.convertStd((val, u, 'group_altitude'), data['usUnits'])
    ...
    def calc_thwIndex(key, data, db_manager=None):
        ...
        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.thw_US(data['outTemp'], data['outHumidity'], data['windSpeed'])
            u = 'degree_F'
        else:
            val = weewx.wxformulas.thw_Metric(data['outTemp'], data['outHumidity'], data['windSpeed'])
            u = 'degree_C'
        return weewx.units.convertStd((val, u, 'group_temperature'), data['usUnits'])
    ...
    def calc_thswIndex(key, data, db_manager=None):
        ...
        if data['usUnits'] == weewx.US:
            val = weewx.wxformulas.thsw_US(data['outTemp'], data['outHumidity'], data['windSpeed'], data['radiation'])
            u = 'degree_F'
        else:
            val = weewx.wxformulas.thsw_Metric(data['outTemp'], data['outHumidity'], data['windSpeed'], data['radiation'])
            u = 'degree_C'
        return weewx.units.convertStd((val, u, 'group_temperature'), data['usUnits'])
...
class SnowRater(weewx.xtypes.XType):
    ...
    def __init__(self, snow_period=900, retain_period=930):

        self.snow_period = snow_period
        self.retain_period = retain_period
        # This will be a list of two-way tuples (timestamp, snow)
        self.snow_events = []
        self.unit_system = None
        self.augmented = False
        self.run_lock = threading.Lock()
    ...
    def add_loop_packet(self, packet):
        """Process LOOP packets, adding them to the list of recent snow events."""
        with self.run_lock:
            self._add_loop_packet(packet)

    def _add_loop_packet(self, packet):
        # Was there any snow? If so, convert the snow to the unit system we are using,
        # then intern it
        if 'snow' in packet and packet['snow']:
            if self.unit_system is None:
                # Adopt the unit system of the first record.
                self.unit_system = packet['usUnits']
            # Get the unit system and group of the incoming snow. In theory, this should be
            # the same as self.unit_system, but ...
            u, g = weewx.units.getStandardUnitType(packet['usUnits'], 'snow')
            # Convert to the unit system that we are using
            snow = weewx.units.convertStd((packet['snow'], u, g), self.unit_system)[0]
            # Add it to the list of snow events
            self.snow_events.append((packet['dateTime'], snow))
    ...
    def get_scalar(self, key, record, db_manager, **option_dict):
        ...
        with self.run_lock:
            ...
            # Sum the snow events within the time window...
            snowsum = sum(x[1] for x in self.snow_events
                          if x[0] > record['dateTime'] - self.snow_period)
            # ...then divide by the period and scale to an hour
            val = 3600 * snowsum / self.snow_period
            # Get the unit and unit group for snowRate
            u, g = weewx.units.getStandardUnitType(self.unit_system, 'snowRate')
            # Form a ValueTuple, then convert it to the unit system of the incoming record
            rr = weewx.units.convertStd(ValueTuple(val, u, g), record['usUnits'])
            return rr

    def _setup(self, stop_ts, db_manager):
        """Initialize the snow event list"""
        ...
        # Query the database for only the events before what we already have
        if self.snow_events:
            first_event = min(x[0] for x in self.snow_events)
            stop_ts = min(first_event, stop_ts)

        # Get all snow events since the window start from the database. Put it in
        # a 'try' block because the database may not have a 'snow' field.
        try:
            for row in db_manager.genSql("SELECT dateTime, usUnits, snow FROM %s "
                ...
                # Skip the row if we already have it in snow_events
                if not any(x[0] == time_ts for x in self.snow_events):
                    self._add_loop_packet({'dateTime': time_ts,
                                           'usUnits': unit_system,
                                           'snow': snow})
        except weedb.DatabaseError as e:
        ...
        # It's not strictly necessary to sort the snow event list for things to work, but it
        # makes things easier to debug
        self.snow_events.sort(key=lambda x: x[0])
...
class HailRater(weewx.xtypes.XType):
    ...
    def __init__(self, hail_period=900, retain_period=930):

        self.hail_period = hail_period
        self.retain_period = retain_period
        # This will be a list of two-way tuples (timestamp, hail)
        self.hail_events = []
        self.unit_system = None
        self.augmented = False
        self.run_lock = threading.Lock()
    ...
    def add_loop_packet(self, packet):
        """Process LOOP packets, adding them to the list of recent hail events."""
        with self.run_lock:
            self._add_loop_packet(packet)

    def _add_loop_packet(self, packet):
        # Was there any hail? If so, convert the hail to the unit system we are using,
        # then intern it
        if 'hail' in packet and packet['hail']:
            if self.unit_system is None:
                # Adopt the unit system of the first record.
                self.unit_system = packet['usUnits']
            # Get the unit system and group of the incoming hail. In theory, this should be
            # the same as self.unit_system, but ...
            u, g = weewx.units.getStandardUnitType(packet['usUnits'], 'hail')
            # Convert to the unit system that we are using
            hail = weewx.units.convertStd((packet['hail'], u, g), self.unit_system)[0]
            # Add it to the list of hail events
            self.hail_events.append((packet['dateTime'], hail))
    ...
    def get_scalar(self, key, record, db_manager, **option_dict):
        ...
        with self.run_lock:
            ...
            # Sum the hail events within the time window...
            hailsum = sum(x[1] for x in self.hail_events
                          if x[0] > record['dateTime'] - self.hail_period)
            # ...then divide by the period and scale to an hour
            val = 3600 * hailsum / self.hail_period
            # Get the unit and unit group for hailRate
            u, g = weewx.units.getStandardUnitType(self.unit_system, 'hailRate')
            # Form a ValueTuple, then convert it to the unit system of the incoming record
            rr = weewx.units.convertStd(ValueTuple(val, u, g), record['usUnits'])
            return rr

    def _setup(self, stop_ts, db_manager):
        """Initialize the hail event list"""
        ...
        # Query the database for only the events before what we already have
        if self.hail_events:
            first_event = min(x[0] for x in self.hail_events)
            stop_ts = min(first_event, stop_ts)

        # Get all hail events since the window start from the database. Put it in
        # a 'try' block because the database may not have a 'hail' field.
        try:
            for row in db_manager.genSql("SELECT dateTime, usUnits, hail FROM %s "
                ...
                # Skip the row if we already have it in hail_events
                if not any(x[0] == time_ts for x in self.hail_events):
                    self._add_loop_packet({'dateTime': time_ts,
                                           'usUnits': unit_system,
                                           'hail': hail})
        except weedb.DatabaseError as e:
        ...
        # It's not strictly necessary to sort the hail event list for things to work, but it
        # makes things easier to debug
        self.hail_events.sort(key=lambda x: x[0])
...
class StdSnowRater(weewx.engine.StdService):
    """Instantiate and register the XTypes extension RainRater."""

    def __init__(self, engine, config_dict):
        """Initialize the RainRater."""
        super(StdSnowRater, self).__init__(engine, config_dict)

        try:
            calc_dict = config_dict['StdWXCalculate']
        except KeyError:
            calc_dict = {}
        snow_period = to_int(calc_dict.get('snow_period', 900))
        retain_period = to_int(calc_dict.get('retain_period', 930))

        self.snow_rater = SnowRater(snow_period, retain_period)
        # Add to the XTypes system
        weewx.xtypes.xtypes.append(self.snow_rater)
    ...
    def shut_down(self):
        """Engine shutting down. """
        # Remove from the XTypes system:
        weewx.xtypes.xtypes.remove(self.snow_rater)

    def new_loop_packet(self, event):
        self.snow_rater.add_loop_packet(event.packet)


class StdHailRater(weewx.engine.StdService):
    """Instantiate and register the XTypes extension HailRater."""

    def __init__(self, engine, config_dict):
        """Initialize the HailRater."""
        super(StdHailRater, self).__init__(engine, config_dict)

        try:
            calc_dict = config_dict['StdWXCalculate']
        except KeyError:
            calc_dict = {}
        hail_period = to_int(calc_dict.get('hail_period', 900))
        retain_period = to_int(calc_dict.get('retain_period', 930))

        self.hail_rater = hailRater(hail_period, retain_period)
        # Add to the XTypes system
        weewx.xtypes.xtypes.append(self.hail_rater)
    ...
    def shut_down(self):
        """Engine shutting down. """
        # Remove from the XTypes system:
        weewx.xtypes.xtypes.remove(self.hail_rater)

    def new_loop_packet(self, event):
        self.hail_rater.add_loop_packet(event.packet)
...
class StdHailRater(weewx.engine.StdService):
    ...
    def __init__(self, engine, config_dict):
        ...
        # Get any user-defined overrides
        try:
            override_dict = config_dict['StdWXCalculate']['HailRater']
        except KeyError:
            override_dict = {}

        # Get the default values, then merge the user overrides into it
        option_dict = weeutil.config.deep_copy(defaults_dict['StdWXCalculate']['HailRater'])
        option_dict.merge(override_dict)

        hail_period = to_int(option_dict.get('hail_period', 900))
        retain_period = to_int(option_dict.get('retain_period', 930))
