...
[StdWXCalculate]
    ...
    retain1_period = 930         # How long to retain snow events. Should be >= snow_period + archive_delay
    hail_period = 900           # Hail rate window
    retain2_period = 930         # How long to retain hail events. Should be >= hail_period + archive_delay
...
class WXCalculate(object):
    ...
    def __init__(self, config_dict, altitude_vt, latitude_f, longitude_f, db_binder=None):
        ...
        # Instantiate a SnowRater to calculate snowRate
        self.snow_rater = SnowRater(to_int(self.svc_dict.get('snow_period', 900)),
                                    to_int(self.svc_dict.get('retain1_period', 930)))

        # Instantiate a HailRater to calculate snowRate
        self.hail_rater = HailRater(to_int(self.svc_dict.get('hail_period', 900)),
                                    to_int(self.svc_dict.get('retain2_period', 930)))
    ...
    def new_loop_packet(self, loop_packet):
        ...
        # Keep the SnowRater up to date:
        self.snow_rater.add_loop_packet(loop_packet, self.db_manager)
        # Keep the HailRater up to date:
        self.hail_rater.add_loop_packet(loop_packet, self.db_manager)
    ...
    def shut_down(self):
        for xtype in [self.pressure_cooker, self.rain_rater, self.snow_rater, self.hail_rater, self.wx_types]:
...
class SnowRater(weewx.xtypes.XType):
    ...
    def __init__(self, snow_period, retain1_period):
        """Initialize the SnowRater.
        Args:
            snow_period: The length of the sliding window in seconds.
            retain_period: How long to retain a snow event. Should be snow_period plus archive_delay.
        """
        self.snow_period = snow_period
        self.retain1_period = retain1_period
        self.snow_events = None
        self.unit_system = None

    def add_loop_packet(self, record, db_manager):
        # Was there any snow? If so, convert the snow to the unit system we are using, then intern it
        if 'snow' in record and record['snow']:
            if self.snow_events is None:
                self._setup(record['dateTime'], db_manager)
            # Get the unit system and group of the incoming snow
            u, g = weewx.units.getStandardUnitType(record['usUnits'], 'snow')
            # Convert to the unit system that we are using
            snow = weewx.units.convertStd((record['snow'], u, g), self.unit_system)[0]
            # Add it to the list of snow events
            self.snow_events.append((record['dateTime'], snow))

        if self.snow_events:
            # Trim any old packets:
            self.snow_events = [x for x in self.snow_events if x[0] >= record['dateTime'] - self.snow_period]
    ...
    def get_scalar(self, key, record, db_manager):
        ...
        snowsum = sum(x[1] for x in self.snow_events if x[0] > record['dateTime'] - self.snow_period)
        # ...then divide by the period and scale to an hour
        val = 3600 * snowsum / self.snow_period
        u, g = weewx.units.getStandardUnitType(record['usUnits'], 'snowRate')
        return ValueTuple(val, u, g)
...
class HailRater(weewx.xtypes.XType):
    ...
    def __init__(self, hail_period, retain2_period):
        ...
        Args:
            ...
            retain_period: How long to retain a hail event. Should be hail_period plus archive_delay.
        """
        self.hail_period = hail_period
        self.retain2_period = retain2_period
        self.hail_events = None
        self.unit_system = None

    def add_loop_packet(self, record, db_manager):
        # Was there any hail? If so, convert the hail to the unit system we are using, then intern it
        if 'hail' in record and record['hail']:
            if self.hail_events is None:
                self._setup(record['dateTime'], db_manager)
            # Get the unit system and group of the incoming hail
            u, g = weewx.units.getStandardUnitType(record['usUnits'], 'hail')
            # Convert to the unit system that we are using
            hail = weewx.units.convertStd((record['hail'], u, g), self.unit_system)[0]
            # Add it to the list of hail events
            self.hail_events.append((record['dateTime'], hail))

        if self.hail_events:
            # Trim any old packets:
            self.hail_events = [x for x in self.hail_events if x[0] >= record['dateTime'] - self.hail_period]
    ...
    def get_scalar(self, key, record, db_manager):
        ...
        hailsum = sum(x[1] for x in self.hail_events if x[0] > record['dateTime'] - self.hail_period)
        # ...then divide by the period and scale to an hour
        val = 3600 * hailsum / self.hail_period
        u, g = weewx.units.getStandardUnitType(record['usUnits'], 'hailRate')
        return ValueTuple(val, u, g)
